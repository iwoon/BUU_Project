...
<form id="myForm" method="post" action="<? echo site_url('permissions/permissions_add/save');?>">
    <div id="permise1" style="margin-bottom:4px;" class="clonePermise">
        ...
        <div id="object1" style="margin-bottom:4px;float:left;" class="clonedObject"><br/>
            ...
            <input type="checkbox" name="operation[]" value="create" <? echo ($permise->create==1)?'checked':'';?>/>เพิ่ม/สร้างใหม่
            <input type="checkbox" name="operation[]" value="read" <? echo ($permise->read==1)?'checked':'';?>/>อ่าน/เข้าถึง
           <input type="checkbox" name="operation[]" value="update" <? echo ($permise->update==1)?'checked':'';?>/> แก้ไข/ปรับปรุง
            <input type="checkbox" name="operation[]" value="delete" <? echo ($permise->delete==1)?'checked':'';?>/>ลบ
            </fieldset>
            <hr/>
        </div>
